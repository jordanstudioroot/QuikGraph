...
namespace QuikGraph.Collections
{
    ...
    public class SortedSet<T> : ISet<T>, ICollection, ISerializable, IDeserializationCallback, IReadOnlyCollection<T>
    {
        ...
        public SortedSet([JBCanBeNull] IComparer<T> comparer)
        {
        ...
        public SortedSet([JBNotNull, ItemCanBeNull] IEnumerable<T> collection)
        ...
        /// <param name="collection">Initial elements to add.</param>
        /// <param name="comparer"><see cref="Comparer{T}"/> to use.</param>
        public SortedSet([JBNotNull, ItemCanBeNull] IEnumerable<T> collection, [JBCanBeNull] IComparer<T> comparer)
            : this(comparer)
        {
        ...
        private void AddAllElements([JBNotNull, ItemCanBeNull] IEnumerable<T> collection)
        {
        ...
        private void RemoveAllElements([JBNotNull, ItemCanBeNull] IEnumerable<T> collection)
        {
            T min = Min;
            T max = Max;
            foreach (T item in collection)
        ...
        private bool InOrderTreeWalk([JBNotNull] TreeWalkPredicate<T> action, bool reverse = false)
        {
            if (_root is null)
                return true;
        ...
        // Otherwise returns false.
        //
        private bool BreadthFirstTreeWalk([JBNotNull] TreeWalkPredicate<T> action)
        {
            if (_root is null)
                return true;
        ...
        public bool Add([JBCanBeNull] T item)
        {
        ...
        /// </summary>
        internal virtual bool AddIfNotPresent([JBCanBeNull] T item)
        {
            if (_root is null)
            {
                // Empty tree
        ...
        internal virtual bool DoRemove([JBCanBeNull] T item)
        {
            if (_root is null)
                return false;
        ...
        private static Node GetSibling([JBCanBeNull] Node node, [JBNotNull] Node parent)
        {
            if (parent.Left == node)
                return parent.Right;
            return parent.Left;
        ...
        private void InsertionBalance(
            [JBCanBeNull] Node current,
            ref Node parent,
            [JBNotNull] Node grandParent,
        ...
        private static bool Is2Node([JBNotNull] Node node)
        {
            Debug.Assert(node != null, "node cannot be null!");
            return IsBlack(node) && IsNullOrBlack(node.Left) && IsNullOrBlack(node.Right);
        }

        private static bool Is4Node([JBNotNull] Node node)
        {
            return IsRed(node.Left) && IsRed(node.Right);
        }

        private static bool IsBlack([JBCanBeNull] Node node)
        {
            return node != null && !node.IsRed;
        }

        private static bool IsNullOrBlack([JBCanBeNull] Node node)
        {
            return node is null || !node.IsRed;
        }

        private static bool IsRed([JBCanBeNull] Node node)
        {
            return node != null && node.IsRed;
        }

        private static void Merge2Nodes([JBNotNull] Node parent, [JBNotNull] Node child1, [JBNotNull] Node child2)
        {
        ...
        private void ReplaceChildOfNodeOrRoot([JBCanBeNull] Node parent, Node child, Node newChild)
        {
            if (parent != null)
            {
        ...
        private void ReplaceNode(
            [JBNotNull] Node match,
            [JBCanBeNull] Node parentOfMatch,
            [JBNotNull] Node successor,
            [JBNotNull] Node parentOfSuccessor)
        {
            if (successor == match)
        ...
        [JBCanBeNull]
        private Node FindNode([JBCanBeNull] T item)
        {
            Node current = _root;
            while (current != null)
            {
        ...
        private int InternalIndexOf([JBCanBeNull] T item)
        {
            Node current = _root;
            int count = 0;
            while (current != null)
        ...
        private static Node RotateLeft([JBNotNull] Node node)
        {
            Node x = node.Right;
            node.Right = x.Left;
            x.Left = node;
        ...
        private static Node RotateLeftRight([JBNotNull] Node node)
        {
        ...
        private static Node RotateRight([JBNotNull] Node node)
        {
            Node x = node.Left;
            node.Left = x.Right;
            x.Right = node;
        ...
        private static Node RotateRightLeft([JBNotNull] Node node)
        {
        ...
        /// </summary>
        private static TreeRotation RotationNeeded([JBNotNull] Node parent, [JBNotNull] Node current, [JBNotNull] Node sibling)
        {
        ...
        private static bool AreComparersEqual(
            [JBNotNull, ItemCanBeNull] SortedSet<T> set1,
            [JBNotNull, ItemCanBeNull] SortedSet<T> set2)
        {
            return set1.Comparer.Equals(set2.Comparer);
        }

        private static void Split4Node([JBNotNull] Node node)
        {
            node.IsRed = true;
        ...
        public void UnionWith([JBNotNull, ItemCanBeNull] IEnumerable<T> other)
        {
            if (other is null)
        ...
        private static Node ConstructRootFromSortedArray(
            [JBNotNull, ItemCanBeNull] IList<T> arr,
            int startIndex,
            int endIndex,
            [JBCanBeNull] Node redNode)
        {
            // What does this do?
        ...
        /// </remarks>
        /// <exception cref="ArgumentNullException">If <paramref name="other"/> is null.</exception>
        public void IntersectWith([JBNotNull, ItemCanBeNull] IEnumerable<T> other)
        {
            if (other is null)
                throw new ArgumentNullException(nameof(other));
        ...
        private void IntersectWithEnumerable([JBNotNull, ItemCanBeNull] IEnumerable<T> other)
        {
        ...
        /// </remarks>
        /// <exception cref="ArgumentNullException">If <paramref name="other"/> is null.</exception>
        public void ExceptWith([JBNotNull, ItemCanBeNull] IEnumerable<T> other)
        {
            if (other is null)
                throw new ArgumentNullException(nameof(other));
        ...
        /// </summary>
        public bool SetEquals([JBNotNull, ItemCanBeNull] IEnumerable<T> other)
        {
            if (other is null)
                throw new ArgumentNullException(nameof(other));
        ...
        /// <returns></returns>
        public int RemoveWhere([JBNotNull] Predicate<T> predicate)
        {
...
// If serialization is not supported, throw an exception. Created these methods to avoid compiler errors
// in Unity 2019.1.7f1.
// TODO: Find a way to refactor this class to avoid compiler errors with Unity in the first place.
    void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context) {
        throw new System.NotImplementedException("Serialization is not supported in this environment.");
    }
    ...
    void IDeserializationCallback.OnDeserialization(object sender) {
        ...
        public struct Enumerator : IEnumerator<T>
        {
...
#endif
            internal Enumerator([JBNotNull] SortedSet<T> set)
            {
                _tree = set;
            ...
            internal Enumerator([JBNotNull] SortedSet<T> set, bool reverse)
            {
                _tree = set;
                _version = _tree._version;
        ...
        /// </remarks>
        public bool TryGetValue([JBCanBeNull] T equalValue, out T actualValue)
        {
